...
import {
  ...
  Linkedin, Phone, Zap, Target, Calendar
} from 'lucide-react';
...
const initialCampaigns = [
  { id: 1, name: 'Q2 Fintech Decision Makers', type: 'email', status: 'active', goal: 'Generate 50 qualified meetings', target: 'Fintech CTOs, Nigeria + Ghana', total_contacts: 850, sent_count: 612, open_rate: 38.4, click_rate: 12.2, reply_rate: 8.7, conversion_rate: 3.2, tags: ['fintech', 'q2'] },
  { id: 2, name: 'LinkedIn ABM - Series B CEOs', type: 'linkedin', status: 'active', goal: 'Book 20 enterprise demos', target: 'Series B+ CEOs, East Africa', total_contacts: 320, sent_count: 320, open_rate: 68.2, click_rate: 22.4, reply_rate: 14.1, conversion_rate: 5.6, tags: ['abm', 'linkedin', 'enterprise'] },
  { id: 3, name: 'WhatsApp Product Launch Africa', type: 'whatsapp', status: 'completed', goal: 'Drive 500 signups', target: 'All segments — Africa-wide', total_contacts: 2100, sent_count: 2100, open_rate: 91.2, click_rate: 34.8, reply_rate: 21.3, conversion_rate: 8.9, tags: ['launch', 'africa'] },
  { id: 4, name: 'Mid-Market Nurture Journey', type: 'multi-channel', status: 'active', goal: 'Move 30 contacts to proposal stage', target: 'Mid-market companies, West Africa', total_contacts: 430, sent_count: 280, open_rate: 44.6, click_rate: 18.9, reply_rate: 11.2, conversion_rate: 4.1, tags: ['nurture', 'mid-market'] },
  { id: 5, name: 'Q3 Webinar — GTM in Africa', type: 'event', status: 'scheduled', goal: 'Register 200 attendees', target: 'Revenue leaders, pan-Africa', total_contacts: 0, sent_count: 0, open_rate: 0, click_rate: 0, reply_rate: 0, conversion_rate: 0, tags: ['webinar', 'event'] },
];
...
const emptyForm = { name: '', type: 'email', goal: '', target: '', total_contacts: '', tags: '' };

function CampaignModal({ campaign, onClose, onSave, aiGenerating, onAIGenerate, aiPrompt, setAiPrompt }) {
          ...
          <div className="p-4 rounded-xl bg-primary/5 border border-primary/20 mb-2">
            ...
            <div className="flex gap-2">
              <Input value={aiPrompt} onChange={e => setAiPrompt(e.target.value)}
                placeholder="e.g. Email campaign for fintech CFOs in Lagos focused on ROI"
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
              placeholder="Q2 Fintech Outbound" className="bg-secondary/50 border-border/60" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select value={form.type} onValueChange={v => setForm({ ...form, type: v })}>
                ...
                <SelectContent>
                  {['email', 'linkedin', 'whatsapp', 'sms', 'multi-channel', 'event'].map(t => (
          ...
          <div>
            ...
            <Input value={form.goal} onChange={e => setForm({ ...form, goal: e.target.value })}
              placeholder="Generate 30 qualified meetings" className="bg-secondary/50 border-border/60" />
          </div>
          <div>
          ...
          <div>
            ...
            <Input value={form.tags} onChange={e => setForm({ ...form, tags: e.target.value })}
              placeholder="fintech, q2, outbound" className="bg-secondary/50 border-border/60" />
          </div>
...
Return JSON with: name (string), type (one of: email/linkedin/whatsapp/multi-channel), goal (string), target (string), tags (array of strings).
    ...
    <div className="min-h-screen" style={{ background: '#f8fafc' }}>
      ...
      {showModal && (
        <CampaignModal
          ...
          onClose={() => { setShowModal(false); setEditingCampaign(null); setAiPrompt(''); }}
          onSave={saveCampaign}
