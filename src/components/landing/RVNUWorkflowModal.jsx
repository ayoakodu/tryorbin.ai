...
import { X, ArrowLeft, ArrowRight, Sparkles, Check, Zap, Mail, MessageCircle, Linkedin, Phone, Users, Building2, TrendingUp, Target, CheckCircle2, Loader2 } from 'lucide-react';
...
const mockAccounts = [
  ...
  { name: 'Paymob', industry: 'Fintech', country: 'Egypt', signal: 'Using Flutterwave', tier: 'High Fit', score: 88 },
          ...
          <div className="space-y-2">
            {prompts.map((p, i) => (
              ...
              >
                ...
                <span className="text-xs leading-relaxed">{p.label}</span>
              </button>
            ))}
          </div>
        </div>
            ...
            {mockAccounts.map((a, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{a.industry} · {a.country}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${a.tier === 'High Fit' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                    {a.tier}
                  </span>
                  <p className="text-[10px] text-slate-400 mt-0.5">Score {a.score}</p>
                </div>
                <span className="text-[10px] px-2 py-1 rounded bg-slate-100 text-slate-500 hidden md:block">{a.signal}</span>
              </motion.div>
            ))}
          </motion.div>
        )}
          ...
          <div className="space-y-2">
            {workflowStages.map((s, i) => (
              <div key={i} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border text-sm ${
                ...
                <span>{s.icon}</span>
                <span className={`text-xs font-medium ${s.status === 'pending' ? 'text-slate-400' : 'text-slate-800'}`}>{s.label}</span>
                <span className="ml-auto">
                  {s.status === 'completed' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />}
                  {s.status === 'active' && <Loader2 className="w-3.5 h-3.5 text-emerald-600 animate-spin" />}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 p-6 overflow-auto flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-800">{channels[activeChannel].label} Sequence — Moove Africa</span>
          </div>
          ...
          {channels[activeChannel].steps.map((step, i) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">AI-generated · Day {i === 0 ? 1 : i * 3}</p>
              </div>
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${i === 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                {i === 0 ? 'Sent' : 'Queued'}
              </span>
            </motion.div>
          ))}
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            {[['5 accounts', 'Enrolled'], ['62%', 'Open Rate'], ['3 replies', 'Received']].map(([v, l]) => (
              <div key={l} className="bg-white rounded-lg p-2 border border-slate-200">
                <p className="text-sm font-bold text-emerald-600">{v}</p>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2 px-6 pt-5 pb-0">
          ...
          <button
            onClick={onClose}
            className="ml-3 w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors flex-shrink-0"
          >
