...
// ─── EMAIL EDITOR ─────────────────────────────────────────────────────────────
function EmailEditor({ step, onUpdate, draft, onDraftChange, allSteps, stepIndex, isNew, sequenceName }) {
  ...
  const prevEmailSubjects = (allSteps || [])
    .slice(0, stepIndex)
    .filter(s => (STEP_TYPE_MAP[s.subtype] || s.type) === 'email' && s.subject)
    .map((s, i) => `Step ${i + 1}: "${s.subject}"`);
...
${prevEmailSubjects.length > 0 ? `- Previous email subjects in this sequence:\n${prevEmailSubjects.join('\n')}` : ''}
...
<HTML email body using only <p> tags>`;
  ...
  // Auto-generate for new empty email steps
  useEffect(() => {
    if (isNew && !step.body && !autoGeneratedRef.current) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex flex-col gap-1 px-5 py-2.5 bg-gradient-to-r from-emerald-50/50 to-white border-b border-slate-100 flex-shrink-0">
        ...
        {generateError && (
          <p className="text-[10px] text-red-500 leading-tight pl-8">{generateError}</p>
        )}
      </div>
...
// ─── MAIN MODAL ───────────────────────────────────────────────────────────────
export default function StepModal({ step, index, isNew, onSave, onClose, allSteps, sequenceName }) {
